...
const StripePaymentForm = ({ amount, onSuccess, method, clientSecret }: StripePaymentProps) => {
  const stripe = useStripe();
  const [error, setError] = useState<string | null>(null);
  const [processing, setProcessing] = useState(false);
  const [pixInfo, setPixInfo] = useState<{ qrCode: string; copyPaste: string } | null>(null);

  useEffect(() => {
    if (method === 'pix' && stripe) {
      handlePix();
    }
  }, [method, stripe]);
  ...
  const handlePix = async () => {
    ...
    const { error, paymentIntent } = await (stripe as any).confirmPixPayment(clientSecret, {
      payment_method: {
        billing_details: {
          name: 'Cliente Doce&Magia', 
        },
      },
    });
    ...
    if (error) {
      ...
      setProcessing(false);
    } else if (paymentIntent && (paymentIntent as any).next_action?.pix_display_qr_code) {
      setPixInfo({
        qrCode: (paymentIntent as any).next_action.pix_display_qr_code.data,
        copyPaste: (paymentIntent as any).next_action.pix_display_qr_code.data,
      });
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="text-center space-y-6">
      {processing ? (
        <div className="py-10 animate-pulse text-pink-400 font-bold">Gerando sua chave PIX segura...</div>
      ) : pixInfo ? (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-3xl shadow-inner border border-pink-100 inline-block mx-auto">
            <QRCodeSVG value={pixInfo.qrCode} size={200} />
          </div>
          <div className="space-y-2">
            <p className="text-[10px] font-black text-brown-300 uppercase tracking-widest">Código Copia e Cola</p>
            <div className="flex gap-2">
              <input 
                readOnly 
                value={pixInfo.copyPaste} 
                className="bg-pink-50 border border-pink-100 rounded-xl p-3 text-[10px] font-mono flex-1 outline-none"
              />
              <button 
                onClick={() => {
                  navigator.clipboard.writeText(pixInfo.copyPaste);
                  alert('Copiado! ✅');
                }}
                className="bg-pink-400 text-white p-3 rounded-xl hover:bg-pink-600 transition-colors"
              >
                <ClipboardList size={18} />
              </button>
            </div>
          </div>
          <div className="bg-yellow-50 p-3 rounded-xl border border-yellow-100 text-[10px] text-yellow-700 font-bold flex items-center gap-2">
            <Clock size={14} /> Válido por 10 minutos. O pedido será processado após a confirmação.
          </div>
        </div>
      ) : (
...
function CookieApp() {
  ...
  const handleSendOrder = async () => {
    ...
    try {
      const response = await fetch('/api/create-payment-intent', {
        ...
        body: JSON.stringify({
          amount: finalAmount,
          paymentMethodTypes: formData.paymentMethod === 'pix' ? ['pix'] : ['card'],
          installments: formData.paymentMethod === 'card_credit' ? formData.installments : 1
        }),
      });
      ...
      // If card, confirm right here
      if (['card_credit', 'card_debit'].includes(formData.paymentMethod)) {
        if (!stripe || !elements) return;
        
        const cardElement = elements.getElement(CardNumberElement);
        if (!cardElement) throw new Error('Dados do cartão não encontrados');
        ...
        if (error) {
          ...
          finalizeOrder(actualDiscountValue, pointsRedeemed, paymentIntent.id);
        }
      } else {
        // If PIX, we need to show the modal to get the QR code
  ...
  return (
    <div className="max-w-[430px] mx-auto bg-pink-50 min-h-screen relative overflow-x-hidden pb-24 selection:bg-pink-100 selection:text-pink-700">
                      ...
                      {['card_credit', 'card_debit'].includes(formData.paymentMethod) && (
                        ...
                        >
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="p-3.5 bg-pink-50/30 rounded-xl border border-pink-100">
                          ...
                          <p className="text-[9px] text-brown-300 mt-2 text-center uppercase font-bold tracking-widest opacity-60">
                            🔒 Dados protegidos pela Stripe. Não salvamos seu cartão.
                          </p>
                        </motion.div>
                      )}
      ...
      <AnimatePresence>
        {paymentVisible && paymentClientSecret && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            >
              ...
              <StripePaymentForm 
                amount={(() => {
                  const currentDeliveryFee = (formData.deliveryType === 'delivery' && !formData.isScheduled) ? settings.deliveryFee : 0;
                  const potentialDiscount = userProfile ? Math.min(cartTotal, userProfile.points * settings.realPerPoint) : 0;
                  const discount = (usePoints && settings.loyaltyEnabled) ? potentialDiscount : 0;
                  const total = cartTotal + currentDeliveryFee - discount;
                  return formData.isScheduled ? (total / 2) : total;
                })()}
                ...
                onSuccess={(id) => {
                  ...
                  const actualDiscountValue = (usePoints && settings.loyaltyEnabled) ? Math.min(cartTotal, potentialDiscount) : 0;
                  const pointsRedeemed = actualDiscountValue / settings.realPerPoint;
                  finalizeOrder(actualDiscountValue, pointsRedeemed, id);
                }}
              />
            </motion.div>
          </motion.div>
